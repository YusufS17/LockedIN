# Phase 3: Room & Contract - Context

**Gathered:** 2026-06-13
**Status:** Ready for planning

> **Renumbered 2026-06-13:** Originally drafted as Phase 2. A new **Phase 2: Character &
> Onboarding** was inserted before this; Room & Contract is now **Phase 3**. Decision IDs
> (D-13..D-20) are unchanged. See D-19 note below — the cozy isometric room is no longer
> fully deferred (user raised room polish to a top priority; the reusable room view is now
...
This phase delivers the "commitment + social pressure" half of the core loop becoming tangible on screen. It does NOT include the live study session, focus tracking, interruptions, or the reveal (Phases 4–6), nor the buildable world/edit (Phase 7). Character creation/onboarding and the reusable avatar + isometric room rendering are built in **Phase 2** and consumed here.
...
Out of scope (defer / other phases): custom contract configuration, multiple rooms / real lobby, friend invites, the full live session HUD with avatars animating at desks (Phase 4), real networking, real payments. (The reusable cozy isometric room view and avatars themselves are built in Phase 2 and consumed here.)
</domain>
...
- **D-18:** Participants are shown in a **roster/list** (you + the three bots) with per-participant status (waiting · joining… · accepted ✓). Avatars reuse the **pixel-art avatar component built in Phase 2** (ONB-03) — the user's created character plus seeded bot avatars (Maya, Leo, Sam).

### Freeze moment + room visual
- **D-19:** *(REVISED 2026-06-13 — user raised room polish to a top priority.)* The cozy isometric pixel-art study-room view is **established as a reusable component in Phase 2** (ONB-05), not deferred. Phase 3 (this phase) **uses that room** — the contract/roster can sit against the cozy room so rooms look polished here too, not only at the Phase 4 session HUD. Keep the build lean: reuse the Phase 2 room view rather than rebuilding it. (Supersedes the original "defer the room to the session phase" decision.)
...
- `.planning/ROADMAP.md` §"Phase 3: Room & Contract" — goal, 5 success criteria, requirement IDs
...
### Visual reference
- `.planning/reference/study-room-mockup.png` — the cozy isometric study-room mockup. The reusable room view + avatars are built in **Phase 2** (ONB-03/ONB-05) and used here; the full animated session HUD is **Phase 4**. Keep Phase 3 screens visually consistent with it (same design tokens, warm dark-academia palette).
...
- `LockedIN/Services/MockFocusControlAdapter.swift` — not used in this phase, but the `Task.sleep` scripted-emitter pattern is the model for the bot accept choreography (D-17).

### Established Patterns
- `@Observable AppStore` injected via `.environment()` at `WindowGroup` root — Phase 3 screens read/observe AppStore; navigation likely via `NavigationStack`/`NavigationPath` (per CLAUDE.md) home → contract → frozen.
...
- **Avatars animating at desks** in the live session — is **Phase 4** (session HUD); the reusable static room view + avatar component themselves are built in **Phase 2** (ONB-03/05) and grown by the world system in **Phase 7**.
- **Richer bot personalities** (banter, distinct behaviours) — surfaces in the **session (Phase 4/5)** and **reveal (Phase 6)**, where Sam cracks and becomes Biggest Culprit.
...
- **Persisting the 18+ confirmation / room state** — this phase keeps session state in-memory; persistence is reserved for world + coins only (per PROJECT.md).

None of the above pulled into Phase 3 scope — discussion stayed within the commitment-ritual boundary.
</deferred>
